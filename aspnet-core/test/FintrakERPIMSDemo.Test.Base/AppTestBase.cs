...
using FintrakERPIMSDemo.Test.Base.TestData;
...
namespace FintrakERPIMSDemo.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="FintrakERPIMSDemoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(FintrakERPIMSDemoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<FintrakERPIMSDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FintrakERPIMSDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<FintrakERPIMSDemoDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<FintrakERPIMSDemoDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FintrakERPIMSDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FintrakERPIMSDemoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FintrakERPIMSDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FintrakERPIMSDemoDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<FintrakERPIMSDemoDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<FintrakERPIMSDemoDbContext, Task<TResult>> func)
        {
            TResult result;
